...
# Compatibility with PyTorch Lightning 2.0+ and 1.x
try:
...
try:
    from pytorch_lightning.utilities.rank_zero import rank_zero_only, rank_zero_experiment
except ImportError:
    try:
...
def train_dataframewrappers(
    ...
    trainer = pl.Trainer(
        devices="auto" if gpus == 0 or gpus is None else gpus,
...
def _get_prob_pair_dataframe(trainer, model, compared, threshold):
    ...
    if isinstance(model, SiameseFeatureModule):
        ...
        from freestyl.supervised.siamese.sequential.data import make_dataloader as SequentialDataLoader
        make_dataloader = SequentialDataLoader
...
def get_df_prediction(
    ...
    if comparator is not None:
        ...
        comparator_labels = comparator.get_labels(as_list=False)
        comparator_idx = torch.arange(comparator_vectors.shape[0])
